// Inline SVG strings for the icons we need. Paths copied from lucide-react
// so the look matches the original site exactly. Use as innerHTML with the
// `icon` class wrapper applied by the parent.

const SVG = (paths, viewBox = "0 0 24 24") =>
  `<svg xmlns="http://www.w3.org/2000/svg" viewBox="${viewBox}" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon" aria-hidden="true">${paths}</svg>`;
...
export const icons = {
  ...
  printer: SVG('<polyline points="6 9 6 2 18 2 18 9"/><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect width="12" height="8" x="6" y="14"/>'),
...
export function injectIcon(node, name) {
...
export function iconHtml(name, extraClass = "") {
  ...
  if (!extraClass) return icons[name];
  return icons[name].replace('class="icon"', `class="icon ${extraClass}"`);
}
